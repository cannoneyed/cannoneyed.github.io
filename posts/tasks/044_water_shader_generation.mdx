...
We need to automatically generate in a step-by-step manner the water shader mask tiles for all water+shore quadrants in the quadrants db. Using the script defined in src/isometric_nyc/e2e_generation/detect_water_tiles.py, we need to be able to do two things:
...
4. Update the src/e2e_generation/app.py to be able to show the water tiles, by adding a new drop-down menu that determines what tile to show (either render, generations, or water_mask)


## Part 2 - Manual water mask generation

We need to modify the src/isometric_nyc/e2e_generation/app.py to be able to generate water mask tiles - we need to add a new entry key "is_water_mask" in the app_config.json that, if that model is used to generate, will save the generated output quadrant data to the `water_mask` column in the generation db.
