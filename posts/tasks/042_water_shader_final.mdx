...
To make this dataset, we'll need a new script called "generate_water_mask_tile.py" in the src/e2e_generation - for now, copy the logic of `generate_nano_banana` since we'll be calling a nano banana model with a modified prompt. Ensure that the script takes the following extra params
...
Script created at: `src/isometric_nyc/e2e_generation/generate_water_mask_tile.py`

**Usage Examples:**

```bash
# Generate water mask for tile at (0,0) using pixel art (default):
uv run python src/isometric_nyc/e2e_generation/generate_water_mask_tile.py \
