...
To make this dataset, we'll need a new script called "generate_dark_mode.py" in the src/e2e_generation - for now, copy the logic of `generate_water_mask_tile` since we'll be calling a nano banana model with a modified prompt (which I'll manuall add later).

**Implementation:** `src/isometric_nyc/e2e_generation/generate_dark_mode.py`

Usage:
```bash
# Generate dark mode for a single tile
uv run python src/isometric_nyc/e2e_generation/generate_dark_mode.py \
