...
title: isometric nyc
date: 01/01/2026
description: isometric pixel art map of NYC
thumbnail: /img/projects/thumbnail-isometric-nyc.jpg
---

![isometric nyc](/img/projects/thumbnail-isometric-nyc.jpg)
...
I’m going to lead with my biggest takeaway: I wound up writing almost no code for this project. I couldn’t have picked a better time to start this project - the release of [Gemini 3](https://blog.google/products-and-platforms/products/gemini/gemini-3/) and [Opus 4.5](https://www.anthropic.com/news/claude-opus-4-5) along with the maturing platforms of [Cursor](https://cursor.com/) and [Claude Code](https://www.anthropic.com/claude-code) marked a true inflection point for the craft of software.

I’d been deep into agentic coding for the past few years and I’ve had to dramatically recalibrate my understanding of software after this project. Much more on this later…
...
And so I started this project as I start almost every project now - [by asking Gemini]((/docs/isometric-nyc-initial-gemini)).

### / NYC City Data

My initial strategy was to use 3D CityGML data from a variety of sources to render a “whitebox” view of individual tiles. I found a few sources ([NYC 3D](https://www.asg.ed.tum.de/gis/projekte/new-york-city-3d/), [3DCityDB](https://github.com/3dcitydb), [NYC CityGML](http://www.3dcitydb.net/3dcitydb/fileadmin/public/datasets/NYC/NYC_buildings_CityGML_LoD2/NYC_Buildings_LoD2_CityGML.zip)), downloaded some data, and then set cursor up to build out a renderer. And very quickly I had something that worked pretty well.

I can’t stress how big of a deal this is - I’ve never worked with CityGML data before and GIS is notoriously finicky and complex. With a lot of back and forth with the agent correcting for coordinate projection systems and geometry labeling schema, I was pretty quickly able to get a renderer set up that could output 

![whitebox](/img/projects/whitebox.png)

I quickly set up a [marimo](https://marimo.io/) notebook to start testing out my plan with Nano Banana Pro and discovered a number of issues. Long story short, there was a bit too much inconsistency between the “whitebox” geometry and the top-down satellite imagery, and Nano Banana was prone to too much hallucination in resolving these differences.
...
**\<\!-- whitebox \+ render images /\>**

### / Image Generation

After a bit of prompt fiddling, I was able to get Nano Banana Pro to generate tiles in my preferred style fairly reliably. But generating image assets with Nano Banana has a few big issues:
...
So I decided to fine-tune a smaller, faster, cheaper model. I opted to try fine-tuning a Qwen/Image-Edit model on the (wonderful) [oxen.ai](http://oxen.ai) service and created a training dataset of \~40 input/output pairs. The fine-tuning took \~4 hours and cost \~12 bucks, and I was pretty happy with the results\!

**\<\!-- fine-tuning pair images /\>**

**tasks/005\_oxen\_api.md**
...
**tasks/006\_infill\_dataset.md**  
**tasks/009\_inpainting.md**  
**tasks/010\_omni\_generation\_dataset.md**
...
Keeping this in mind, I opted to design an end-to-end generation application to facilitate the process of generating the tiles. Experience shows that you’ll hit edge cases and hit them fast (and oh did we), so it’s good practice to start small before scaling things up. I whipped up a spec and had the agent generate a simple system for driving generation:
...
**\<\!-- web-application screencast\>**

**007\_e2e\_generation.md**  
**008\_e2e\_generation\_pixels.md**  
**012\_generation\_rules.md**  
This determines the template logic of what can or can’t be generated in terms of neighboring tiles - we need to be able to eliminate seams, so generations must have correct boundaries  
**013\_generation\_app.md**  
Generation app creates an e2e web app for generating tile quadrant data programmatically. As we’ll see, this tool wound up being the main driver of generations\!

And lo and behold, with a bit of back-and-forth I had a working web application for progressively generating tile data. 

> Aside on tool building  
...
But at the end of the day, the last 10% will take up 90% of the time and, as always, the difference between good enough and great is the amount of love you put into the work.
...
[Oxen.ai](http://Oxen.ai) is a wonderful service - automating and abstracting away all of the fiddly bits of fine-tuning and deploying models and managing training data. But inference through the platform was rather expensive and rather slow - if I wanted to scale up the process to generate *all* of the NYC map we’d need to make the models significantly faster (or more parallelizable) and cheaper.

So I opted to export the weights from Oxen to my own rented GPU+VMs using [Lambda AI](https://lambda.ai/) (another fantastic service). I’ve been training and deploying models for a *long* time, and I remember the horrors of getting models to run on commodity hardware. But add this process to the long list of things that AI coding agents *magically solve*. 
...
\> Aside - Again, software engineering doesn’t go away in the age of AI - it just moves up the ladder of abstraction. I still had to spec out the behavior of the generation queues and logic which incorporated all of the subtle domain-specific logic, but I no longer cared about *any* of the code that implemented it. I’m serious - I’ve never even looked at it.
...
First off, it was *extremely* difficult to get the agents to implement and understand an efficient tiling algorithm. The rules for generation are fairly simple - no quadrant may be generated such that a “seam” will be present. 
...
**015\_automatic\_generation.md**  
**016\_generation\_playbook.md**  
This attempts to figure out how to automate generation via an efficient tile/infill algorithm… This actually wound up being problematic for a number of reasons, a) the algorithm was too annoying to specify and/or cursor/gemini/claude couldn’t nail it and b) there needs to be more human debugging / approval than I’d like.  
**019\_strip\_plan.md**  
Another attempt at automating the quadrant generation plan for a given strip of quadrants…

However I found myself quickly confronted with another, more insidious failure mode. The generations themselves often had issues, and no matter what I did I couldn’t devise a system for cheaply and efficiently auditing and/or correcting them.

**\<\!-- failure modes graphic here\>**

In most cases, even the smartest image models like Gemini 3 pro couldn’t reliably assess most of the failure modes (such as seams and incorrect tree generation). And even when the model could assess these issues, there was no way to deploy it reliably at a scale and speed that wouldn’t make the process intractable. 
...
Once again the same lesson applies - even though you’re not writing any more code, you’re still doing software engineering.
...
Of course, software engineering rules still apply. Entropy is real; as you add features, complexity grows, and without architecture, you accumulate tech debt. But here’s the thing: for throwaway tools—debuggers, visualizers, script runners—**code quality doesn't really matter.**
