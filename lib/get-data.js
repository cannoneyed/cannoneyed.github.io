...
const DOCS_DIR = path.join(process.cwd(), "posts/docs");
...
// Docs functions
export function getAllDocsPath() {
  const allMdxFiles = getMdxFiles(DOCS_DIR);
  return allMdxFiles.map((parsedFile) => {
    return {
      params: {
        id: parsedFile.name,
...
export function getDocData(id) {
  const fullPath = path.join(DOCS_DIR, id + MDX_EXTENSION);
